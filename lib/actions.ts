...
export async function createPost(postData: PostData) {
  try {
    ...
    if (!writeToken) {
      throw new Error('SANITY_API_WRITE_TOKEN is not configured. Please add it to your environment variables with Editor or Administrator permissions.');
    }
    ...
    const existingPosts = await client.fetch<Post[]>(
    ...
    if (!response.ok) {
      ...
      if (response.status === 403 || result?.error?.type === 'mutationError') {
        throw new Error(`Permission denied: Your SANITY_API_WRITE_TOKEN does not have "create" permission. Please generate a new token with Editor or Administrator role in your Sanity project settings.`);
      }
